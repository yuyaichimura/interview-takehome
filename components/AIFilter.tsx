...
export const AIFilter = ({ onFilterChange }: { onFilterChange: (filters: Filters) => void }) => {
  ...
  const { isLoading } = useQuery('filters', fetchFilters, {
    enabled: fetchQuery,
    onSuccess: (data: any) => {
    ...
    <div className="w-10/12 mb-12">
      ...
      <div className="w-full">
        <form onSubmit={handleFilterSubmit}>
          ...
          <button
            type="submit"
            className="min-h-[auto] w-1/12 ml-3 rounded-lg border-4 px-2 py-2 border-zinc-500 bg-white hover:border-sky-600 text-gray-800"
            disabled={isFetchingFilter || !query || isLoading}
          >
            {isLoading ?
            <svg
              aria-hidden="true"
